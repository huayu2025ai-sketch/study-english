...
export default function Home() {
  ...
  function toggleTheme() {
    setDark((value) => {
      const next = !value;
      window.localStorage.setItem("weather-club-theme", next ? "dark" : "light");
  ...
  return <main className="library-home">
    <header className="topbar"><Link className="brand" href="/"><span>☁</span> Little Weather Club</Link><div className="library-count">USBORNE BEGINNERS · 35 BOOKS</div><div className="header-actions"><button className="theme-toggle" onClick={toggleTheme} aria-label="切换深色模式">{dark ? "☀️" : "🌙"}<span>{dark ? "浅色" : "深色"}</span></button><div className="streak">📚 <b>Pick a book</b></div></div></header>
