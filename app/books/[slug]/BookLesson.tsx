...
import { stageInfo, type Book } from "../../lib/books";

const commonQuestions = [
  ["Which word belongs to this book’s topic?", "topic"],
  ["What is a good way to learn a new word?", "say it aloud"],
  ["What should you do after reading a page?", "tell one new fact"],
];
...
export function BookLesson({ book }: { book: Book }) {
  ...
  const questions = useMemo(() => [
    ...
    { question: commonQuestions[0][0], options: [book.keywords[0] ?? "topic", "banana", "pencil"], answer: 0 },
    { question: commonQuestions[1][0], options: ["Say it aloud", "Hide the word", "Skip every page"], answer: 0 },
    { question: commonQuestions[2][0], options: ["Tell one new fact", "Close the book immediately", "Forget the word"], answer: 0 },
  ], [book]);
  ...
  function next() {
    if (selected === questions.length - 1) setDone(true);
    else setSelected((value) => (value === null ? 0 : value + 1));
  }
